...
$dateSelectedMail = Carbon::parse($selectedMail->getTime());
@endphp
        <!DOCTYPE html>
<html lang="ru">
<head>
    ...
    <title>MailInterceptor @xman12</title>
</head>
<body>
    ...
    <div class="aside">
        ...
        <div class="aside-content">
            @foreach($mails as $key => $mail)
                @php
                    $dateMail = Carbon::parse($mail->getTime());
                @endphp
                <div class="message-wrap pt-2 pb-1 px-3 @if($id === $key) active-message @endif">
                    <div class="message">
                        ...
                        >
                            ...
                            <div>
                                <p class="message-time captionS m-0">{{ $dateMail->format('Y-m-d H:i:s') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="message-details">
                                <div>
                                    ...
                                    <p class="message-data bodyM m-0 white60 d-inline-block">
                                        <span class="message-day">{{ $dateSelectedMail->format('d') }}</span>
                                        <span class="message-month">{{ $dateSelectedMail->format('M') }}</span>
                                        <span class="message-year">{{ $dateSelectedMail->format('Y') }}</span> в
                                        <span class="message-hour">{{ $dateSelectedMail->format('H') }}</span>:<span
                                                class="message-minute">{{ $dateSelectedMail->format('i') }}</span
                                        >
                                    </p>
                                </div>
                            </div>
                          ...
                          >
                            ...
                            >
                                <div class="accordion-body p-0 pt-1">
                                    <p class="bodyS white60 m-0">Message
                                        Id: {{ $selectedMail->getHeaders()->getMessageId() }}</p>
                                    <p class="bodyS white60 m-0">
                                        MIME-Version: {{ $selectedMail->getHeaders()->getMimeVersion() }}</p>
                                    <p class="bodyS white60 m-0">
                                        Content-Type: {{ $selectedMail->getHeaders()->getContentType() }}</p>
                                    <p class="bodyS white60 m-0">
                                        Content-Transfer-Encoding: {{ $selectedMail->getHeaders()->getContentTransferEncoding() }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
